...
function djs_html5_datetime (f_params)
{
	var f_return = null;

	if ((f_params !== undefined)&&('id' in f_params)) { var f_jquery_objects = jQuery("#" + f_params.id).toArray (); }
	else { f_jquery_objects = jQuery('input[type="date"],input[type="datetime"],input[type="datetime-local"],input[type="month"],input[type="time"]').filter(":visible").toArray (); }

	var f_jquery_check;

	if ((f_jquery_objects.length < 1)||(('valueAsDate' in f_jquery_objects[0])&&('valueAsNumber' in f_jquery_objects[0])&&(!isNaN (f_jquery_objects[0].valueAsNumber)))) { f_jquery_check = false; }
	else
	{
		...
		{
			djs_html5_datetime_ready = f_jquery_objects;
			var f_deferreds,f_path = "";
			...
			if ('datetime_lang' in f_params) { f_deferreds.push (jQuery.ajax ({ cache:true,dataType:'script',url:f_path + "ext_jquery/jquery.ui.datepicker." + f_params.datetime_lang + ".min.js" })); }
			f_return = jQuery.when.apply(jQuery,f_deferreds).done (djs_html5_datetime_replace);
		}
		else
		{
			if (typeof (djs_html5_datetime_ready) == 'boolean') { f_jquery_check = true; }
			else { djs_html5_datetime_ready.concat (f_jquery_objects); }
		}
	}

	if (f_jquery_check) { djs_html5_datetime_replace (f_jquery_objects); }
	return f_return;
}

function djs_html5_datetime_replace (f_jquery_objects)
{
	if (typeof (djs_html5_datetime_ready) != 'boolean')
	{
		f_jquery_objects = djs_html5_datetime_ready;
...
function djs_html5_datetime_set (f_params)
{
	if (('id' in f_params)&&('value' in f_params))
	{
		if (djs_html5_datetime_ready === true) { jQuery("#" + f_params.id).progressbar ('value',f_params.value); }
